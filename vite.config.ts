...
export default ({ mode }) => {
  ...
  return defineConfig({
    ...
    base: '/',
    publicDir: 'public',
    logLevel: 'info', // Adjust console output verbosity.
    // development server configuration
    server: {
      // Vite 3 now defaults to 5173, but you can override it with the port option.
      port: 3333,
      //! Update this proxi setup while implementing API calls
      proxy: {
        '/interceptor': {
          ...
          secure: false,
          ws: true,
          rewrite: (path) => path.replace(/^\/interceptor/, ''),
        },
      },
    },
    /**
    ...
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          name: 'App Name - Lorem ipsum dolor',
          short_name: 'App Name',
