<script setup lang="ts">
import type { RouteLocationAsString } from 'unplugin-vue-router'

export type VTabsType = 'boxed' | 'toggle' | 'rounded'
export type VTabsAlign = 'centered' | 'right'
export interface VTabsItem {
  label: string
  value: string
  icon?: string
  to?: RouteLocationAsString
}
export interface VTabsProps {
  tabs: VTabsItem[]
  selected?: string
  type?: VTabsType
  align?: VTabsAlign
  slider?: boolean
  slow?: boolean
  hasExtra?: boolean
}

const emit = defineEmits<{
  (e: 'update:selected', value: string): void
}>()
const props = withDefaults(defineProps<VTabsProps>(), {
  selected: undefined,
  type: undefined,
  align: undefined,
})

const activeValue = ref(props.selected)
const sliderClass = computed(() => {
  if (!props.slider) {
    return ''
  }

  if (props.type === 'rounded') {
    if (props.tabs.length === 3) {
      return 'is-slider'
    }
    if (props.tabs.length === 2) {
      return 'is-slider'
    }

    return ''
  }

  if (!props.type) {
    if (props.tabs.length === 3) {
      return 'is-squared is-slider'
    }
    if (props.tabs.length === 2) {
      return 'is-squared is-slider'
    }
  }

  return ''
})

function toggle(value: string) {
  activeValue.value = value
}

watch(
  () => props.selected,
  (value) => {
    activeValue.value = value
  }
)

watch(activeValue, (value) => {
  emit('update:selected', value)
})
</script>

<template>
  <div class="tabs-wrapper" :class="[sliderClass]">
    <div
      class="tabs-inner mb-4 is-flex is-align-items-center is-justify-content-space-between"
    >
      <div
        class="tabs"
        :class="[
          props.align === 'centered' && 'is-centered',
          props.align === 'right' && 'is-right',
          props.type === 'rounded' && !props.slider && 'is-toggle is-toggle-rounded',
          props.type === 'toggle' && 'is-toggle',
          props.type === 'boxed' && 'is-boxed',
        ]"
      >
        <ul>
          <li
            v-for="(tab, key) in tabs"
            :key="key"
            :class="[activeValue === tab.value && 'is-active']"
          >
            <slot
              name="tab-link"
              v-bind="{
                activeValue,
                tab,
                key,
                toggle,
              }"
            >
              <RouterLink
                tabindex="0"
                class="is-center-flex"
                :to="tab.to ?? ''"
                @keydown.enter="toggle(tab.value)"
                @click="toggle(tab.value)"
              >
                <VIcon v-if="tab.icon" :icon="tab.icon" class="tab-icon mr-2" />
                <span>
                  <slot
                    name="tab-link-label"
                    v-bind="{
                      activeValue,
                      tab,
                      key,
                      toggle,
                    }"
                  >
                    {{ tab.label }}
                  </slot>
                </span>
              </RouterLink>
            </slot>
          </li>
          <li v-if="sliderClass" class="tab-naver"></li>
        </ul>
      </div>
      <div v-if="props.hasExtra">
        <slot name="tab-extra"></slot>
      </div>
    </div>

    <div class="tab-content is-active">
      <Transition :name="props.slow ? 'fade-slow' : 'fade-fast'" mode="out-in">
        <div>
          <slot
            name="tab"
            v-bind="{
              activeValue,
            }"
          ></slot>
        </div>
      </Transition>
    </div>
  </div>
</template>

<style lang="scss" scoped>
.tabs {
  margin-bottom: unset !important;
}
</style>
